    ...
    editGridRow: function (rowid, p) {
      ...
      return this.each(function () {
        ...
        var gID = $t.p.id,
          ...
          IDs = {
            themodal: 'editmod' + gID, modalhead: 'edithd' + gID,
            modalcontent: 'editcnt' + gID, scrollelm: frmgr
          },
        ...
        if ($("#" + $.jgrid.jqID(IDs.themodal)).html() !== null) {
          ...
          if ($($t).hasClass('patient-table')) {
            flr = `
            <tr class='service-search'>
              <td class='search-passport-label CaptionTD'>Паспорт</td>
              <td class='search-passport DataTD' colspan='${maxCols * 2}'>&nbsp;<input maxlength="2" name='passport-serie' class='passport-serie FormElement ui-widget-content ui-corner-all' id='search-passport-serie' style='width: 20px; text-transform: uppercase' />
                <input maxlength="7" name='passport-number' class='passport-number only-numbers FormElement ui-widget-content ui-corner-all' id='search-passport-number' style='width: 107px' />
              </td>
            </tr>
            <tr class='service-search'>
              <td class='search-pinfl-label CaptionTD'>ПИНФЛ</td>
              <td class='search-pinfl DataTD' colspan='${maxCols * 2}'>&nbsp;<input maxlength="14" minlength="14" name='pinfl' class='pinfl only-numbers FormElement ui-widget-content ui-corner-all' id='search-pinfl' style='margin-bottom: 10px;' />
                <button onclick="GetPersonDocData(this)" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-icon-only" role="button" aria-disabled="false" title="Calendar" style="font-size: 69%;">
                  <span class="ui-button-icon-primary ui-icon ui-icon-search">
                  </span><span class="ui-button-text">Calendar</span>
                </button>
              </td>
            </tr>
            <tr class='service-search'>
              <td colspan="3" style="height: 10px; border-top: 1px solid gray;"></td>
            </tr>`;
            $(tbl).prepend(flr);

            $('.passport-serie').on('keyup', function () {
              let val = $(this).val();
              if (val.length > 2) {
                val = val.substr(0, 2);
                $(this).val(val);
          ...
          if ($($t).hasClass('patient-table')) {
            ...
            $('.passport-number').on('keydown', function () {
              var key = event.keyCode || event.charCode;
              if ($(this).val() == '' && key == 8 || key == 46) {
                $('.passport-serie').focus();
              }
            });
            initNumberInput();
            initPinflInput();
