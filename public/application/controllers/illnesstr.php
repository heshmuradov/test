...
class Illnesstr extends MyController
{
    function Illnesstr()
    {
        parent::MyController();
    ...
    function index()
    {
        ...
        $data['royhatuz'] = $this->db->query('SELECT * FROM spr_royhat where id in (9,6)')->result();
        ...
        $this->load->view("/illness/illnesstr", $data);
        }
        else {
    ...
    function search()
    {
        ...
        try {
            ...
            $trdate = isset($_POST['ftrdate']) ? $_POST['ftrdate'] : null;
            $trdate_gacha = isset($_POST['ftrdate_gacha']) ? $_POST['ftrdate_gacha'] : null;
            $id_sabab = isset($_POST['fid_sabab']) ? $_POST['fid_sabab'] : null;
            ...
            if (!empty($trdate) && !empty($trdate_gacha)) {
                $hfilter = $hfilter . " and h.trdate BETWEEN to_date('$trdate', 'yyyy-mm-dd') and to_date('$trdate_gacha', 'yyyy-mm-dd')";
            }
            ...
            if (!empty($id_royhat)) {
                ...
                $hfilter = $hfilter . " and h.id_royhat in (9,6)";
            }
            if (!empty($seriya)) {
    ...
    <ol>
        ...
        <li><a href="/illnesstr/download/<?php echo $p['id']; ?>"><?php echo $p['name']; ?></a></li>
        <?php endforeach; ?>
    </ol>
    <?php
    ...
    function search_excel()
    {
        ...
        if (!empty($id_royhat)) {
            ...
            $hfilter = $hfilter . " and h.id_royhat in (9)";
        }
        if (!empty($seriya)) {
            $hfilter = $hfilter . " and upper(h.seriya) like upper('%$seriya%')";
        }
