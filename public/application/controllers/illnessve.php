...
class Illnessve extends MyController
{
    function Illnessve()
    {
        parent::MyController();
    ...
    function index()
    {
        if ($this->session->userdata['admin_type'] != 5) {
            ...
            $data['royhatuz'] = $this->db->query('SELECT * FROM spr_royhat where id in (7)')->result();
            ...
            $this->load->view("/illness/illnessve", $data);
        } else {
    ...
    function search()
    {
        ...
        try {
            ...
            if (!empty($id_royhat)) {
                ...
                $hfilter = $hfilter . " and h.id_royhat in (7)";
            }
            if (!empty($seriya)) {
